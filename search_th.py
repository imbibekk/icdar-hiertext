...
for th_word in [0.4, 0.5, 0.6, 0.7, 0.8]:
    for th_line in [0.4, 0.5, 0.6, 0.7, 0.8]:
        for th_para in [0.4, 0.5, 0.6, 0.7, 0.8]:
            save_path = f'w_{th_word}_{th_line}_{th_para}'
            os.system(f'CUDA_VISIBLE_DEVICES=1 python ensemble.py --sub_path1 output/cfg8/submission/validation.pt --sub_path2 output/cfg7/submission/validation.pt --sub_path3 output/cfg10/submission/validation.pt --sub_path4 output/cfg11/submission/validation.pt --config_name {config_name} --save_path {save_path} --th_word {th_word} --th_line {th_line} --th_para {th_para}')
