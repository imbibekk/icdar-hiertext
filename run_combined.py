...
if __name__ == '__main__':
    ...
    cfg.component_min_probs = [args.th_word, args.th_line, args.th_para]
    ...
    #print(out1['batch_masks_prob'].shape, out2['batch_masks_prob'].shape, out3['batch_masks_prob'].shape) # torch.Size([1724, 6, 2048, 2048]), torch.Size([1724, 3, 2048, 2048])
    ...
    shrink_factor = cfg.vertices_shrink_factor
    submission = batch_masks2submission(
        outs,
        out1['batch_image_ids'],
        out1['batch_image_heights'].tolist(),
        out1['batch_image_widths'].tolist(),
        min_prob=cfg.component_min_probs,
        component_min_area=cfg.component_min_area,
        mask_dilate_iter=cfg.mask_erode_iter,
        vertices_expand_factor=shrink_factor/(1-shrink_factor),
        vertices_expand_max_dist=outs.shape[2] * cfg.vertices_shrink_max_dist_factor,
        promotion_min_score=cfg.promotion_min_score,
        device=args.device
    )
